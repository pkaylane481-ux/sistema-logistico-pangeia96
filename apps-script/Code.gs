...
function carregarBaseCompleta() {
  validarUsuario_();
  const planilha = SpreadsheetApp.getActiveSpreadsheet();

  function registros(tabela) {
    const aba = planilha.getSheetByName(ABAS[tabela]);
    if (!aba) throw new Error("Aba não encontrada: " + ABAS[tabela]);
    return ler_(aba).registros.map(limpar_);
  }

  return {
    usuarios: registros("usuarios"),
    operadores: registros("operadores"),
    atividades: registros("atividades"),
    transportadoras: registros("transportadoras"),
    marketplaces: registros("marketplaces"),
    motivos: registros("motivos"),
    produtos: registros("produtos"),
    trocas: registros("trocas_devolucoes"),
    devolucoes: registros("devolucoes_logistica"),
    devolucaoMarketplace: registros("devolucao_marketplace"),
    falhas: registros("falhas"),
    prioridades: registros("pedidos_prioridade"),
    produtividade: registros("produtividade"),
    slas: registros("slas")
  };
}
